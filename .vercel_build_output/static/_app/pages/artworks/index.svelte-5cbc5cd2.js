...
import { B as L, a as B, f as q } from '../../chunks/Button-8fecae15.js';
import { S as G } from '../../chunks/Section-0453228e.js';
const R = [
...
class Z extends t {
	constructor(t) {
		super(), e(this, t, Y, K, a, { slideNo: 0, totalSlides: 1, image: 2, altTag: 3, attr: 4 });
	}
}
function z(t) {
	let e, a, s, r, o, g;
...
class Q extends t {
	constructor(t) {
		super(), e(this, t, F, z, a, { thumbImg: 0, altTag: 1, titleLink: 2, id: 3, selected: 4 });
	}
}
function W(t) {
...
function nt(t) {
	...
	(a = new Z({
		...
		(g = new tt({ props: { caption: J[t[0]].name } })),
		g.$on('prevClick', t[3]),
		g.$on('nextClick', t[2]);
	let d = J,
		p = [];
	...
	return {
		...
		p(t, [e]) {
			...
			2 & e && (n.image = t[1].imgurl),
				...
				a.$set(n);
			const s = {};
			if ((1 & e && (s.caption = J[t[0]].name), g.$set(s), 17 & e)) {
				let a;
				for (d = J, a = 0; a < d.length; a += 1) {
					const n = et(t, d, a);
					p[a]
						? (p[a].p(n, e), k(p[a], 1))
...
function st(t, e, a) {
	...
	return (
		(t.$$.update = () => {
			1 & t.$$.dirty && console.log(s), 1 & t.$$.dirty && a(1, (n = J[s]));
		}),
		[
			s,
			n,
			() => {
				a(0, s === J.length - 1 ? (s = 0) : (s += 1));
			},
			() => {
				a(0, 0 === s ? (s = J.length - 1) : (s -= 1));
			},
			r,
			(t) => r(t)
		]
...
function pt(t) {
	let e, a, $, f, d, p, E, D, T, L, G, R, _, A, M, O;
	(E = new ct({
		props: {
			image: t[1].imgurl,
			altTag: t[1].name,
		...
		(T = new ut({ props: { caption: J[t[0]].name } })),
		T.$on('prevClick', t[3]),
		T.$on('nextClick', t[2]);
	let U = J,
		H = [];
	for (let n = 0; n < U.length; n += 1) H[n] = dt(ht(t, U, n));
	const K = (t) =>
		w(H[t], 1, 1, () => {
			H[t] = null;
		});
	return {
		...
		p(t, [e]) {
			...
			2 & e && (a.image = t[1].imgurl),
				...
				E.$set(a);
			const n = {};
			if ((1 & e && (n.caption = J[t[0]].name), T.$set(n), 17 & e)) {
				let a;
				for (U = J, a = 0; a < U.length; a += 1) {
					const n = ht(t, U, a);
					H[a]
						? (H[a].p(n, e), k(H[a], 1))
						: ((H[a] = dt(n)), H[a].c(), k(H[a], 1), H[a].m(G, null));
				}
				for (N(), a = U.length; a < H.length; a += 1) K(a);
				S();
			}
		},
		i(t) {
			if (!A) {
				...
				j(() => {
					_ && _.end(1), R || (R = V(e, B, { x: -300 })), R.start();
				}),
					(A = !0);
			}
		},
